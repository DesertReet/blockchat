...
public final class BlockChatHud {
	...
	public static boolean isCaptureToastActive() {
		Minecraft client = Minecraft.getInstance();
		pruneEndedCaptureToasts(client.getToastManager());
	...
	public static void dismissCaptureToast() {
		Minecraft client = Minecraft.getInstance();
		ToastManager manager = client.getToastManager();
		pruneEndedCaptureToasts(manager);
	...
	private static void pruneEndedCaptureToasts(ToastManager manager) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (manager.getToast(SystemToast.class, id) != null) {
				ACTIVE_CAPTURE_TOAST_IDS.addLast(id);
			}
		}
	}
