...
public final class ChatTab implements BlockChatTab {
	...
	private void renderFace(GuiGraphics graphics, ChatFriendEntry entry, int drawX, int drawY) {
		if (BlockChatClientCompat.drawPlayerFace(graphics, entry.liveSkin, drawX, drawY, FACE_SIZE)) {
			return;
		}

		ResourceLocation faceTexture = SkinHelper.getFaceTexture(entry.skinUrl, entry.uuid);
		if (faceTexture != null) {
	...
	private record ChatFriendEntry(String uuid, String username, String skinUrl, Object liveSkin, ChatStatus status) {
	}
