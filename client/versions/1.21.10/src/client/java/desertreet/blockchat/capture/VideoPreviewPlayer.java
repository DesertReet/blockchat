...
public final class VideoPreviewPlayer {
	...
	public void start() {
		...
		texture = BlockChatImageCompat.createDynamicTexture("blockchat_video_preview", decodeWidth, decodeHeight, false);
		textureId = ResourceLocation.fromNamespaceAndPath(
	...
	private void uploadFrame(byte[] rgbaData) {
		...
		for (int y = 0; y < decodeHeight; y++) {
			for (int x = 0; x < decodeWidth; x++) {
				...
				BlockChatImageCompat.setPixel(pixels, x, y, (a << 24) | (b << 16) | (g << 8) | r);
				idx += 4;
			}
		}
		texture.upload();
